...
/**
 * <P>
 * This example prints a pdf file or directory of files using JPS in the XMP
 * Scope:<b>(All)</b>
 */
public class SilentPrint {
	...
	final private void printAllCopies() {
		 ...
		 * 
		 * JPS also has a function to set number of copies but not at a document
		 * level
		 */
	...
	private void printPages() {
		...
		try {
			...
			if (!matchFound) {
				...
				this.reportError("Printer " + printer
						+ " not supported. Options=" + list);
			} else {

				/**
				 * sample code to see if tray vaues supported and select (not
				 * used by default)
				 */
				if (tray != null) {
					boolean hasTrayValues = printJob.getPrintService()
					...
					if (hasTrayValues) {
						...
						Map<String, Object> possibleValues = new HashMap<String, Object>();
						// Class category = Media.class;
						Object o = printJob.getPrintService()
						...
						Class[] c = printJob.getPrintService()
				...
				if (debugCode) {
					...
					for (int i = 0; i < count1; i++)
						logMessage(i + " " + attribs[i].getName() + " "
								+ attribs[i].toString());
				}
				// <end-13>
				 ...
				 */
				// decode_pdf.setTextPrint(PdfDecoder.NOTEXTPRINT); //normal
				// mode - only needed to reset
				// decode_pdf.setTextPrint(PdfDecoder.TEXTGLYPHPRINT);
				// //intermediate mode - let Java create Glyphs if font matches
				// decode_pdf.setTextPrint(PdfDecoder.TEXTSTRINGPRINT); //try
				// and get Java to do all the work
