 ...
 */
public class ButtonsPanel extends JPanel implements MouseListener, KeyListener,
	...
	private JButton buildButton(String label, String icon, boolean enabled,
			String keyStroke) {
	...
	public void setAccelerators(JButton button, String keyStroke) {
		...
		button.getInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW).put(
				KeyStroke.getKeyStroke(accel), "answer");
	...
	public void setEnabled(String name, boolean bool) {
		if (Hbuttons.containsKey(name)) {
			/*
			 * Comentada estas lineas funcionaban para el manejo del foco en las
			 * subformas, ya que habia inconvenientes con el mismo
			 */
			/*
			 * if (!bool) {
	...
	private void callEvent(String action) throws InvocationTargetException,
			NotFoundComponentException, IOException {
		...
		if (obj instanceof Components) {
			...
			for (Object object : vec) {
				...
				String idManualTransaction = element
						.getChildText("idManualTransaction");
				...
				if ("transaction".equals(type)) {
					...
					String pathTemplate = element
							.getChildText("printerTemplate");
					String jarDirTemplates = EmakuParametersStructure
							.getJarDirectoryTemplates();
					try {
						...
						if (url != null) {
							...
							Attribute ATType = rootTemplate
									.getAttribute("type");
							Attribute ATSilent = rootTemplate
									.getAttribute("silent");
							Attribute ATCopies = rootTemplate
									.getAttribute("copies");
							Attribute ATprinter = rootTemplate
									.getAttribute("printer");

							boolean silent = ATSilent != null ? ATSilent
									.getBooleanValue() : false;
							int copies = ATCopies != null ? ATCopies
									.getIntValue() : 1;
							String typePrinter = ATType.getValue();
							String printer = ATprinter != null
									&& !ATprinter.getValue().trim().equals("") ? ATprinter
									.getValue()
									: null;
							System.out.println("template path: "
									+ jarDirTemplates + pathTemplate);
							if ("PLAIN".equals(typePrinter)) {
								plainManager.setNdocument(lastNumber);
								plainManager.processPostScript(rootTemplate,
										printJob);
								if (plainManager.isSuccessful()) {
									...
									System.out
											.println("========================================");
									ImpresionType IType = plainManager
											.getImpresionType();
									ByteArrayInputStream IStream = plainManager
											.getStream();
									new PrintingManager(IType, IStream, silent,
											copies, printer, 0, 0);
								}
								...
								postScriptManager.processPDF(rootTemplate,
										printJob);
								ByteArrayInputStream IStream = postScriptManager
										.getStream();
								ImpresionType IType = postScriptManager
										.getImpresionType();
								int width = postScriptManager.getWidth();
								int height = postScriptManager.getHeight();
								new PrintingManager(IType, IStream, silent,
										copies, printer, width, height);
								 ...
								 * postScriptManager.getStream(); ImpresionType
								 * IType = ImpresionType.POSTSCRIPT; int width =
								 * postScriptManager.getWidth(); int height =
								 * postScriptManager.getHeight(); new
								 * PrintingManager(IType,IStream, silent,
								 * copies,printer,width,height);
								 * postScriptManager = new
								 * PostScriptManager(lastNumber);
								 */
								postScriptManager.load(rootTemplate, printJob);
								new PrintingManager(postScriptManager, silent,
										copies, printer);
								postScriptManager = new PostScriptManager(
										postScriptManager.getNdocument());
							...
							System.out.println("Plantilla " + jarDirTemplates
									+ pathTemplate + " no encontrada");
							JOptionPane.showInternalMessageDialog(GFforma,
									"NO SE ENCONTRO LA PLANTILLA DE IMPRESION");
						}
	...
	@SuppressWarnings("unchecked")
	private synchronized void builTransaction(Vector<Components> vec,
			String action, Element printJob, String idManualTransaction)
		...
		for (int i = 0; i < vec.size(); i++) {
			...
			if ("getMultiPackage".equals(comp.getMethod())) {
				if (comp.isContainsArgs()) {
					multielementos = (Element[]) GFforma.invokeMethod(comp
							.getDriver(), comp.getMethod(),
							new Class[] { Element.class }, new Object[] { comp
									.getArgs() });
				} else {
					multielementos = (Element[]) GFforma.invokeMethod(comp
							.getDriver(), comp.getMethod());
				...
				if (comp.isContainsArgs()) {
					elementos = (Element) GFforma.invokeMethod(
							comp.getDriver(), comp.getMethod(),
							new Class[] { Element.class }, new Object[] { comp
									.getArgs() });
				} else {
		...
		if (printJob == null) {
			String namePackage;
			if (!("REPORT".equals(action) || "EXCEL".equals(action) || "RELOADDATA"
					.equals(action))) {
				namePackage = "TRANSACTION";
			} else {
				System.out.println("action: "
						+ Hbuttons.get(action).getTypePackage());
			...
			if (pack.size() > 0 || !namePackage.equals("TRANSACTION")) {
				try {
					formatPackageStructure(pack, namePackage,
							idManualTransaction, Hbuttons.get(action)
									.getIdReport());
	...
	/*
	 * public void actionPerformed(ActionEvent e) { System.out.println("When: " +
	 * e.getWhen()); actionEvent(e.getActionCommand()); }
	 */
					...
					try {
						...
						if (Hform.containsKey(action)) {
							...
							for (int i = 0; i < vforms.size(); i++) {
								new GenericForm(GFforma, (Element) vforms
										.get(i));
							}
						}
						JButton button = null;
	...
	private void formatPackageStructure(Vector pack, String packageName,
			String idManualTransaction, String idReport)
		...
		if ("TRANSACTION".equals(packageName)) {
			...
			if (idReport != null) {
				...
				throw new MalformedProfileException(Language
						.getWord("MALFORMED_PROFILE")
						+ GFforma.getIdTransaction());
			}
			...
			if (idReport != null) {
				...
				jarFile.setText(EmakuParametersStructure
						.getParameter("jarFile"));
				jarDirectory.setText(EmakuParametersStructure
						.getParameter("jarDirectory"));
				transaction.getRootElement().addContent(jarFile);
				transaction.getRootElement().addContent(jarDirectory);
				driver.setText(idReport);
				XLSReceiver xlsReceiver = new XLSReceiver(GFforma,
						idTransaction);
			...
			if (idReport != null) {
				driver.setText(idReport);
				TextReportViewer plainViewer = new TextReportViewer(GFforma,
						idReport);
		...
		for (int i = 0; i < pack.size(); i++) {
			transaction.getRootElement()
					.addContent((Element) pack.elementAt(i));
		}
		...
		int times = 0;
		while (!actionFinish && !badActionFinish
				&& "TRANSACTION".equals(packageName)) {
			try {
				if (times <= 5000) {
					Thread.sleep(100);
				} else {
	...
	public void setEnabled(Element args) {
		...
		if (name != null && !"".equals(name) && state != null
				&& !"".equals(state)) {
			if (Hbuttons.containsKey(name)) {
				Hbuttons.get(name).getButton().setEnabled(
			...
			JOptionPane.showInternalMessageDialog(GFforma.getDesktopPane(),
					Language.getWord("ERROR_PARAMETERS_BUTTONS"), Language
							.getWord("ERROR_MESSAGE"),
					JOptionPane.ERROR_MESSAGE);
		}
	}
