...
public class EmakuImportExcelFile extends JPanel implements Couplable,
		ActionListener {
	...
	public EmakuImportExcelFile(GenericForm GFforma, Document doc) {
		...
		JBopen = new JButton();
		JBopen.setIcon(new ImageIcon(this.getClass().getResource(
				"/icons/ico_excel.png")));
		...
		class ImportarExcel extends Thread {
			...
			public void run() {
				try {
					 ...
					 * 
					 * element = new Element("table"); //for (Sheet
					 * sheet:workbook.getSheets()) { Sheet sheet =
					 * workbook.getSheet(0); for(int i=3;i<sheet.getRows();i++) {
					 * Cell[] celdas = sheet.getRow(i); rows = new
					 * Element("row"); int clean = 0; for(Cell celda:celdas) {
					 * //for (int j=2;j<5;j++) { //Cell celda = celdas[j]; if
					 * (celda.getType()== CellType.NUMBER) { NumberCell nc =
					 * (NumberCell) celda; rows.addContent(new
					 * Element("col").setText(""+nc.getValue())); } else {
					 * rows.addContent(new
					 * Element("col").setText(celda.getContents())); }
					 * 
					 * if (celda.getType() == CellType.EMPTY) { clean++; } } if
					 * (celdas.length==clean) { break; }
					 * element.addContent(rows); } //} notificando();
					 */
					...
					for (int i = 3; i < sheet.getRows(); i++) {
						...
						for (Cell celda : celdas) {
							rows.addContent(new Element("col").setText(celda
									.getContents()));
						}
