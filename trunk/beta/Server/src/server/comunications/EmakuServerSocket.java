...
public class EmakuServerSocket {
    ...
    public EmakuServerSocket() throws IOException{
            ...
            SSclient.bind(new InetSocketAddress(ConfigFileHandler.getClientSocket()));
            SSadmin.bind(new InetSocketAddress(ConfigFileHandler.getAdminSocket()));
            ...
            LogAdmin.setMessage(Language.getWord("SOCKET_SERVER_OPEN") + " "
                    + ConfigFileHandler.getClientSocket(), ServerConstants.MESSAGE);
            LogAdmin.setMessage(Language.getWord("SOCKET_SERVER_OPEN") + " "
                    + ConfigFileHandler.getAdminSocket(), ServerConstants.MESSAGE);
                    ...
                    while (iterador.hasNext()) {
                        ...
                        try {
                            if (clave.isAcceptable()) {
                                ...
                                if (getSocketsCount() <
                                        ConfigFileHandler.getMaxClients()) {
...
class InfoSocket extends Thread {
    ...
    public void run() {
        try {
            Thread.sleep(5000);
            if (!isLoged()) {
                EmakuServerSocket.removeSock(sock.getChannel());
            }
        }
