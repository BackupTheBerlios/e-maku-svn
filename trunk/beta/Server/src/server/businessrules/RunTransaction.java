...
public class RunTransaction extends Thread {
	...
	public void run() {
		...
		/*
		 * Con esta informaci�n se verifica si el usuario tiene permisos para
		...
		if (SQLFormatAgent.permisoTransaccion(bd, login, transaction_code,
				password)) {
			System.out.println("bd: " + bd + " transaction_code: "
					+ transaction_code + " Driver: "
					+ LinkingCache.getDriver(bd, transaction_code));
			callDriver(LinkingCache.getDriver(bd, transaction_code),
					id_transaction);
		} else
			errorMessage(sock, id_transaction, Language
					.getWord("TRANSACTION_ACCESS_DENIED"));
	}
	...
	private void callDriver(BusinessRulesStructure CLDdriver,
			...
			if (CLDdriver.getDriver() != null) {
				...
				if (CLDdriver.getArg_driver() != null) {
					type_args_constructor = new Class[] { SocketChannel.class,
							Document.class, Element.class, String.class };

					args_constructor = new Object[] { sock,
							CLDdriver.getArg_driver(), pack, id_transaction };
				} else {
					type_args_constructor = new Class[] { SocketChannel.class,
							Element.class, String.class };
					args_constructor = new Object[] { sock, pack,
							id_transaction };
				}
				...
				if (CLDdriver.getMethod() != null) {
					...
					Method meth = cls.getMethod(CLDdriver.getMethod(),
							type_args_method);
					meth.invoke(obj, args_method);
				}
			} else {
				errorMessage(sock, id_transaction, Language
						.getWord("ERR_MODULE_LG")
		 ...
		 */
			...
			type_args_constructor = null;
			type_args_method = null;
			args_constructor = null;
			args_method = null;
			pack = null;
			transaction = null;
			System.gc();
