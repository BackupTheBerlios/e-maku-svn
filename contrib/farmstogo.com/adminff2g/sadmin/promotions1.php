...
<table cellpadding=3 cellspacing=1 border="0">
  ...
  <tr> 
	...
	if (!strcmp($dt["ID_PRODUCTO"],$dx["ID_PRODUCTO_PROMOTION"])) { $sel = " selected"; } 
		?>
		<option value="<?echo $dt["ID_PRODUCTO"]?>" <?echo $sel?>> <?echo strtoupper($dt["TIPO_PRODUCTO"])."-".$dt["SUB_TIPO_PRODUCTO"]?>-<? echo strtoupper($dt["SIZE"]).", ".$dt["PRODUCTO"]." [$".$dt["PRICE"]."]";?></option> 
		<? 
	}                     
      ?>   
  ...
  <tr>       
	...
	mysql_data_seek($rs,0);
		while($dt = mysql_fetch_array($rs)) { 
	$sel = "";
	if (!strcmp($dt["ID_PRODUCTO_BONUS"],$bonus)) { $sel = " selected"; } 
		?>
		<option value="<?echo $dt["ID_PRODUCTO"]?>" <?echo $sel?>> <?echo strtoupper($dt["TIPO_PRODUCTO"])."-".$dt["SUB_TIPO_PRODUCTO"]?>-<? echo strtoupper($dt["SIZE"]).", ".$dt["PRODUCTO"]." [$".$dt["PRICE"]."]";?></option> 
		<? 
	}                     
