...
if ($flag) {
	for ($i=0; $i<count($bunch); $i++) {
		$sql_updateproducts = "
			UPDATE 	PRODUCTOS 
			SET 	UNIT_COST=$bunch[$i]
					, QBCODE='$qbcode[$i]' 
			WHERE 	ID_PRODUCTO=$id[$i]
			";
		mysql_db_query($db,$sql_updateproducts);
		$sql_updateproductsprices = "
			UPDATE 	PRODUCTOS P
					, SIZES S 
			SET 	PRICE_STEM = CASE 
			WHEN 	P.UNIT = 'S' 
					THEN (((P.UNIT_COST * P.STEMS_PER_BUNCH * P.BUNCHES_BOX/ $markup) + S.FEDEX_COST) *1.03)/( P.STEMS_PER_BUNCH * P.BUNCHES_BOX) 
			WHEN 	P.UNIT = 'B' 
					THEN ROUND((((P.UNIT_COST * P.BUNCHES_BOX/ $markup) + S.FEDEX_COST) * 1.03)/( P.BUNCHES_BOX),2)
			END 
			WHERE	P.`ID_SIZE` = S.`ID_SIZE` AND P.`ID_PRODUCTO`=$id[$i]
			";
		//echo $sql_updateproductsprices;	
		mysql_db_query($db,$sql_updateproductsprices) or die(mysql_error());
...
&nbsp;&nbsp;	
	<select name="public"  onchange="this.form.submit()">
				...
				<option value="1" <? if ($public=="1") { echo " selected";} ?>>Public</option>
				<option value="0" <? if ($public=="0") { echo " selected";} ?>>No Public</option>	             
	</select>	
...
if ($public != "") { $spublic = " AND M.public=$public"; }

$qry = "
	SELECT	P.*
			,S.SIZE
			,TP.TIPO_PRODUCTO
			,ST.SUB_TIPO_PRODUCTO
			,M.MARKET
	FROM	PRODUCTOS P
			,SIZES S
			,TIPO_PRODUCTOS TP
			, SUB_TIPO_PRODUCTOS ST
			, MARKETS M 
	WHERE	TP.ID_TIPO_PRODUCTO=P.ID_TIPO_PRODUCTO 
	AND 	ST.ID_SUB_TIPO_PRODUCTO=P.ID_SUB_TIPO_PRODUCTO 
	AND		P.ID_MARKET=M.ID_MARKET 
