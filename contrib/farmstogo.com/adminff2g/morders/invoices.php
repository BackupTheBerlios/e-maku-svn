...
Titulo_Maestro("Orders List / Invoice Pending");
?>
...
if (!$status) { $st = " AND o.ID_ORDER_STATUS=2";}
...
if (mysql_numrows($rs)) {
echo "<b>$dx</b> records found  &nbsp;(<a target=other href=\"https://businesscenter.cybersource.com/sbc/sbc_login.jsp\"><b>Issue Invoice at Bank of America</b></a>)<p>"
...
<tr bgcolor=ccccaa><td align=center colspan=9>
	...
	<input type=button value="Submit" onclick="if (this.form.op.options[this.form.op.selectedIndex].value!='') { OpenW(this.form.op.options[this.form.op.selectedIndex].value+'?f1=<?echo $f1?>&f2=<?echo $f2?>&status=2&kw=<?echo $kw?>','600','400',1)}">
</td>
	</form>
</tr>
 ...
 <input type=submit value="Invoiced" style="font-weight:bold"></td></tr>
</table>
<input type=hidden name=sts value="8">
<input type=hidden name=url value="invoices.php">
