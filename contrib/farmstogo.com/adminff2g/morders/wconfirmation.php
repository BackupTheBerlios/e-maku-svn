...
Titulo_Maestro("Orders List / Waiting Confirmation");
?>
...
if (!$status) { $st = " AND o.ID_ORDER_STATUS=1";}
else if ($status=='ALL') { $st =""; }
else if($status) { $st = " AND o.ID_ORDER_STATUS='$status'";}
if($kw) {$skw = " AND (o.ID_FACTURA LIKE '%$kw%' OR o.SHIPPING_NAME LIKE '%$kw%' OR o.SHIPPING_LAST_NAME LIKE '%$kw%') ";}
...
<table cellpadding=3 cellspacing=1 border=1 bgcolor=cecece>
 ...
 <input type=submit value="Confirmed" style="font-weight:bold"></td></tr>
</table>
<input type=hidden name=sts value="2">
<input type=hidden name=url value="wconfirmation.php">
