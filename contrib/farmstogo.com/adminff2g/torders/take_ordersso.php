...
Titulo_Maestro("Take Standing Orders");
...
<table cellpadding=3 cellspacing=1 border="0" bgcolor=cecece>
 <!-- <tr> 
  ...
  <input type="hidden" name="so" id="so"  value="1"/>
  <tr bgcolor=ffff77> 
    <td colspan="3"> 
	...
	"TIPO_PRODUCTOS T,SUB_TIPO_PRODUCTOS S, PRODUCTOS P, MARKETS M WHERE S.ID_TIPO_PRODUCTO=T.ID_TIPO_PRODUCTO AND M.ID_MARKET = 2 AND ".
	...
	$qx = "select DISTINCT M.MARKET,S.SIZE,P.ID_PRODUCTO,P.PRODUCTO,P.BUNCHES_BOX,round(P.PRICE_BUNCH*P.BUNCHES_BOX,2) AS PRICE from MARKETS M, SIZES S, ".
	"PRODUCTOS P WHERE P.ID_SUB_TIPO_PRODUCTO=$pr AND S.ID_SIZE=P.ID_SIZE "."AND M.ID_MARKET=2 AND ".
