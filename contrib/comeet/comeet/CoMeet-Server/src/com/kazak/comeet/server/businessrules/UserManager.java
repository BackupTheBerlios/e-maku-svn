...
public class UserManager {
	...
	public UserManager(SocketChannel sock, Element args, Element packet, String id) {
		...
		try {
			...
			LogWriter.write("ERROR[SQLNotFoundException]: Ocurrio una falla mientras se procesaba operacion de Usuario [Tipo:"+type+"]");
			LogWriter.write("Causa: " + message);
		} catch (SQLBadArgumentsException e) {
			result = false;
			message = e.getMessage();
			LogWriter.write("ERROR[SQLBadArgumentsException]: Ocurrio una falla mientras se procesaba operacion de Usuario [Tipo:"+type+"]");
			LogWriter.write("Causa: " + message);
		} catch (SQLException e) {
			result = false;
			message = e.getMessage();
			LogWriter.write("ERROR[SQLException]: Ocurrio una falla mientras se procesaba operacion de Usuario [Tipo:"+type+"]");
			LogWriter.write("Causa: " + message);
		}
		if (result) {
